...
def readGraph(filename, level=-1, body=[], label="", graphname="", pathprefix=""):
    ...
    print("Iterate durch nodes wird aufgerufen mit filename ", fname, " pathprefix ", pathprefix, " und lastslash ",
          lastslash)
    tree = ET.parse(fname)
    root = tree.getroot()
    ...
    for child in root:
        if child.tag.endswith("state"):
            ...
            # case: substatemachine in seperate .xml
                ...
                print("\n", "readgraph wird aufgerufen mit src ", src, " und pathprefix ", prefix_tmp, " aus ",
                      root.attrib['initial'])
                (sg, oE, ini) = readGraph(src, level=level + 1, body=detSubBody(level), graphname="cluster_" + src,
                                          pathprefix=prefix_tmp)
                ...
                if level + 1 >= subst_recs:
                    ...
                    for out_edge in oE:
                        for propTrans in child:  # propably transitions
                            if propTrans.tag[len(ns):] == "transition" and propTrans.attrib['event'] == child.attrib[
                                'id'] + "." + out_edge[1]:
                                if 'target' in propTrans.attrib:
                                    target = propTrans.attrib['target']
                                    eventName = out_edge[0][:out_edge[0].find(".")] + "." + out_edge[0] + "." + \
                                                out_edge[1]
                ...
                for each in child:
                    if each.tag[len(ns):] == "transition":
                        ...
                        if "target" in each.attrib:
                            inEdges.append((child.attrib['id'], each.attrib['target'], each.attrib['event'],
                                            detEdgeColor(each.attrib['event'])))
...
def removeDoubles(ed, amount=4):
    ...
    for each in ed:
        for every in doubleless:
            if amount == 4 and each[0] == every[0] and each[1] == every[1] and each[2] == every[2] and each[3] == every[
                3]:
                break
